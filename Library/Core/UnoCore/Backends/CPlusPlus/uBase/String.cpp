...
namespace uBase
{
    static size_t GetFloatLength(const char* str, size_t len)
    {
    ...
    void String::InitLength(size_t len)
    {
    ...
    {
        char buf[32];
        size_t len = sprintf_s(buf, 32, "%d", i);
        Init(buf, len);
    }
    ...
    {
        char buf[64];
        size_t len = sprintf_s(buf, 64, "%f", f);
    ...
    {
        char buf[128];
        size_t len = sprintf_s(buf, 128, "%lf", d);
        Init(buf, GetFloatLength(buf, len));
    }
    ...
    String String::Trim() const
    {
        ...
        while (start < end)
            if (isspace((unsigned char)_data[start]))
                start++;
            else
                break;

        while (end > start)
            if (isspace((unsigned char)_data[end - 1]))
                end--;
            else
                break;
    ...
    String String::ToLower() const
    {
        ...
        for (size_t i = 0; i < _length; i++)
            r._data[i] = (char)(unsigned char)tolower((unsigned char)r._data[i]);
    ...
    String String::ToUpper() const
    {
        ...
        for (size_t i = 0; i < _length; i++)
            r._data[i] = (char)(unsigned char)toupper((unsigned char)r._data[i]);
    ...
    bool String::EndsWith(const String& str) const
    {
        ...
        for (size_t i = _length - str._length; i < _length; i++)
    ...
    int String::CompareTo(const char* str, size_t len) const
    {
        size_t l = _length;
        if (len < l) l = len;
    ...
    String String::Add(const char* str, size_t len) const
    {
        String r;
    ...
    {
        return Equals(str, !str ? 0 : strlen(str));
    }
    ...
    {
        return Add(str, !str ? 0 : strlen(str));
    }
    ...
    bool String::operator <(const char* str) const
    {
        return CompareTo(str, !str ? 0 : strlen(str)) < 0;
    }
