...
namespace Xli
{
    ...
    public:
        ...
        virtual CURLcode SetCurlRequestOptions(CURL* session, const void* content, size_t byteLength)
        {
        ...
        virtual void SendAsync(const void* content, size_t byteLength)
        {
            if (state != HttpRequestStateOpened)
            {
    ...
    private:
        ...
        static int onProgress(void *clientp, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
        {
            ...
            {
                HttpEventHandler* eh = request->client->GetEventHandler();
                if (eh!=0) eh->OnRequestProgress(request, (int)dlnow, (int)dltotal, (dltotal > 0));
            }
            if (ulnow && (request->state == HttpRequestStateSent))
            {
                HttpEventHandler* eh = request->client->GetEventHandler();
                if (eh!=0) eh->OnRequestProgress(request, (int)ulnow, (int)ultotal, (ultotal > 0));
            }
            return 0;
        }
